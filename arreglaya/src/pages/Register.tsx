...
import {Paper,Stack,Typography,Button,Grid,FormControl,InputLabel,Select,MenuItem,Alert,Checkbox,FormControlLabel,Link} from "@mui/material";
...
export default function Register() {
  ...
  const dataOk =
    name.trim().length >= 2 && emailOk && pwOk && password === confirm;

  function next() {
    if (active === 0) setActive(1);
    else if (active === 1) {
      if (!dataOk) {
        setError("Revisá los datos obligatorios.");
        return;
      }
      setError(null);
      setActive(2);
  ...
  async function finish() {
    if (!(acceptTerms && acceptPrivacy)) {
      setError("Debés aceptar Términos y Privacidad.");
      return;
    }
    setError(null);
    setLoading(true);
    try {
      ...
      saveProfile({ name, email, role, barrio, profession });
      setSuccess(true);
          ...
          {active === 1 && (
            <Stack spacing={2}>
              ...
              <PasswordField
                ...
                helperText={
                  !!confirm && password !== confirm ? "No coincide" : ""
                }
              />
              <Stack direction="row" spacing={2}>
                ...
                <Button onClick={next} disabled={!dataOk}>
                  Continuar
                </Button>
              </Stack>
            </Stack>
